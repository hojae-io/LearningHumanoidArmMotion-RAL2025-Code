...
class HumanoidFullModularEnv(HumanoidVanillaEnv):
    ...
    def _init_buffers(self):
        ...
        self.action = torch.zeros(self.num_envs, self.num_leg_joints + self.num_arm_joints, device=self.device)
    ...
    def _compute_generalized_coordinates(self):
        ...
        self.gen_coord_pin_RS = torch.hstack((self.robot.data.root_link_pos_w,
                                              torch.stack(math_utils.euler_xyz_from_quat(self.robot.data.root_link_quat_w), dim=1),
                                              self.robot.data.joint_pos))
        self.gen_coord_pin_RS = brl_utils.convert_gen_vel_from_isaaclab_to_pin(self.gen_coord_pin_RS)
    ...
    def _get_log_data(self):
        robot_idx = 0
        self.action[:, self.action_manager.get_term('leg_joint_pos')._joint_ids] = self.action_manager.action['leg_joint_pos']
        self.action[:, self.action_manager.get_term('arm_joint_pos')._joint_ids] = self.action_manager.action['arm_joint_pos']
        log_data = {
            ...
            'leg_joint_pos_target': brl_utils.convert_joint_order_from_isaaclab_to_pin(self.robot.data.default_joint_pos + self.action)[robot_idx, :self.num_leg_joints],
            ...
            'arm_joint_pos_target': brl_utils.convert_joint_order_from_isaaclab_to_pin(self.robot.data.default_joint_pos + self.action)[robot_idx, self.num_leg_joints:],
            ...
            'gen_coord': self.gen_coord_pin_RS,
            'gen_vel_body': self.gen_vel_body_pin,
        }
        return log_data
...
self.cusadi_CM_fn.evaluate([gen_coord_pin.to('cuda'), gen_vel_body_pin.to('cuda')])
CLM, CAM = self.cusadi_CM_fn.outputs_sparse
self.CM = torch.hstack([CLM, CAM]).float().to(self.device)

self.cusadi_CoM_fn.evaluate([gen_coord_pin.to('cuda')])
CoM = self.cusadi_CoM_fn.outputs_sparse[0].to(self.device)
"""
