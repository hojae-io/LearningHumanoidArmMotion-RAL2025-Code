...
# Pre-defined configs
##
from extensions.humanoid.assets import HUMANOID_FIXED_ARMS_CFG  # isort: skip
...
class MySceneCfg(InteractiveSceneCfg):
    ...
    # robots
    robot: ArticulationCfg = HUMANOID_FIXED_ARMS_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
    # sensors
...
class ActionsCfg:
    ...
    joint_pos = mdp.JointPositionActionCfg(asset_name="robot", joint_names=[".*"]) #! The order of the joints are not correct.
...
class RewardsCfg:
    ...
    class LegRewardsCfg:
        ...
        #     weight=1.0,
        #     params={"asset_cfg": SceneEntityCfg("robot"), "base_height_target": 0.65}
        # )
...
@configclass
class HumanoidVanillaEnvCfg(ManagerBasedRLEnvCfg):
    ...
    # viewer = ViewerCfg(eye=(3.0, .0, 0.5), origin_type='asset_root', asset_name='robot')
...
@configclass
class HumanoidVanillaEnvCfg_PLAY(HumanoidVanillaEnvCfg):
    def __post_init__(self):
        # post init of parent
        super().__post_init__()
